...
from .models import IssuerProvider, KeycloakTokenPayload, UserContext
...
async def _get_jwks(issuer: str, kid: str, valid_issuers: list[str]) -> dict[str, Any]:
    ...
    try:
        ...
        async with httpx.AsyncClient(verify=config.openremote_verify_ssl, timeout=JWKS_REQUEST_TIMEOUT_SECONDS) as client:
...
def _is_excluded_route(path: str, excluded_routes: list[str], base_url: str = "/") -> bool:
    """Check if the path matches any of the excluded routes."""
    for route in excluded_routes:
        # Handle routes that start with "/" by adding base path prefix
        if route.startswith("/"):
            full_route_with_prefix = base_url + route if base_url else route
...
class KeycloakMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        from app.config import config
        if _is_excluded_route(path, self.excluded_routes, config.base_url):
        ...
        # Store user context in request state for access in tools
        # Import here to avoid circular dependency
        try:
            from app.context import set_user_context
            set_user_context(user_context)
        except ImportError:
            # Context module not available, continue without setting
            pass
